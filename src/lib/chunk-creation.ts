...
export async function createChunksForDocument(documentId: string) {
  try {
    ...
    if (successCount > 0) {
      await supabase
        ...
        .update({
          status: "ready",
          error: errorCount > 0 ? `${errorCount} chunks failed` : null,
        })
        .eq("id", documentId);

      console.log("=== DIRECT CHUNK CREATION SUCCESS ===");
      return {
        success: true,
