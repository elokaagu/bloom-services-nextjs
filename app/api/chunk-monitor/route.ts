...
export async function GET(req: NextRequest) {
  try {
    ...
    const summary = {
      ...
      documentsNeedingProcessing: analysis.filter((d) => d.needsProcessing)
        .length,
      documentsCanProcess: analysis.filter((d) => d.canProcess).length,
      processingDocuments: analysis.filter((d) => d.status === "processing")
        .length,
      failedDocuments: analysis.filter((d) => d.status === "failed").length,
      uploadingDocuments: analysis.filter((d) => d.status === "uploading")
        .length,
    };
    ...
    return NextResponse.json({
      ...
      recommendations: {
        ...
        needsAttention:
          analysis.filter((d) => d.status === "failed").length > 0,
        processingInProgress:
          analysis.filter((d) => d.status === "processing").length > 0,
      },
    });
  } catch (error: any) {
