...
export async function POST(req: NextRequest) {
  try {
    console.log("=== COMPREHENSIVE DOCUMENT PROCESSING START ===");
    ...
    try {
      // First, try to trigger the ingest API
      ...
      if (ingestResponse.ok) {
        ...
        if (newChunks && newChunks.length > 0) {
          ...
          console.log(`Document processed successfully with ${newChunks.length} chunks`);
          ...
          return NextResponse.json({
            ...
            chunksPreview: newChunks.slice(0, 2).map((c) => ({
              chunkNo: c.chunk_no,
              textPreview: c.text.substring(0, 100) + "...",
            })),
          });
        } else {
          console.error("No chunks were created despite successful ingest");
          await supabase
            .from("documents")
    ...
    console.error("=== COMPREHENSIVE DOCUMENT PROCESSING ERROR ===", error);
    return NextResponse.json(
      {
        error: error.message,
        stack: error.stack,
