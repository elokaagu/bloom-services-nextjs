...
export async function POST(req: NextRequest) {
  try {
    ...
    // update status
    await supabaseService.from('documents').update({ status: 'processing' }).eq('id', documentId)
    ...
    const { error: insErr } = await supabaseService.from('document_chunks').insert(rows)
    if (insErr) throw insErr

    await supabaseService.from('documents').update({ status: 'ready' }).eq('id', documentId)
    ...
    if (req.body) {
      ...
      try {
        const { documentId } = await req.json()
        if (documentId) await supabaseService.from('documents').update({ status: 'failed', error: e.message }).eq('id', documentId)
      } catch {}
    }
