...
async function questionEmbedding(q: string) {
  const r = await openai.embeddings.create({ model: process.env.EMBEDDING_MODEL!, input: q })
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!workspaceId || !userId || !question) return NextResponse.json({ error: 'missing fields' }, { status: 400 })

    // log query
    const { data: qrow } = await supabaseService
      .from('queries')
      .insert({ workspace_id: workspaceId, user_id: userId, question, model: process.env.GENERATION_MODEL })
      .select('*')
      .single()
    ...
    const { data: retrieved, error } = await supabaseService.rpc('match_chunks', {
    ...
    const prompt = [
      { role: 'system', content: `You are Bloom's internal knowledge assistant. Only answer from the provided CONTEXT. If the answer is not in context, say you don't have enough information.` },
    ...
    const citations = (chunks || []).map((c, i) => ({ index: i+1, chunkId: c.id, documentId: c.document_id }))
